...
function addToMealPlan(recipe) {
  ...
  removeButton.onclick = () => removeFromMealPlan(mealItem); // Corrected: use mealItem for removal
...
function addToShoppingList(recipe) {
  ...
  const removeButton = document.createElement("button");
  removeButton.textContent = "Remove";
  removeButton.onclick = () => removeFromShoppingList(shoppingItem); // Corrected to remove from shopping list
...
function removeFromShoppingList(shoppingItem) {
  // Remove the shopping list item from the DOM
...
//login page
 ...
 // Closing the popup when clicking outside the popup box
